...
export function BedBathPicker({ bedrooms, bathrooms, onChange }) {
    ...
    <div className="space-y-4">
      <div>
        ...
        <div className="grid grid-cols-8 gap-1.5">
          {BEDROOM_OPTS.map((n) => (
            <button
              key={n}
              type="button"
              onClick={() => onChange({ bedrooms: n, bathrooms })}
        ...
        {bedrooms && (
          <button
            type="button"
            onClick={() => onChange({ bedrooms: null, bathrooms })}
      ...
      <div>
        <label className="text-xs uppercase tracking-wider text-stone-500 font-mono mb-2 block">
          Bathrooms{" "}
          <span className="normal-case text-stone-400 text-[10px]">
            (.5 = half bath)
          </span>
        </label>
        <div className="grid grid-cols-9 gap-1.5">
          {BATHROOM_OPTS.map((n) => (
            <button
              key={n}
              type="button"
              onClick={() => onChange({ bedrooms, bathrooms: n })}
        ...
        {bathrooms && (
          <button
            type="button"
            onClick={() => onChange({ bedrooms, bathrooms: null })}
            className="text-[11px] text-stone-500 hover:text-stone-700 mt-1.5 font-mono"
          >
            Clear
          </button>
