...
export function StatCard({ label, value, unit, highlight, accent, onClick }) {
  ...
  const content = (
    <>
      <div
        className={`text-xs uppercase tracking-wider font-mono mb-1 flex items-center justify-between ${highlight ? "text-stone-400" : accent ? "text-amber-700" : "text-stone-500"}`}
      >
        <span>{label}</span>
      ...
      <div className="flex items-baseline gap-1.5">
        <span
          className={`text-2xl font-serif ${accent ? "text-amber-900" : ""}`}
        >
          {value}
        </span>
        {unit && (
          <span
            className={`text-xs font-mono ${highlight ? "text-stone-400" : "text-stone-500"}`}
          >
            {unit}
          </span>
        )}
